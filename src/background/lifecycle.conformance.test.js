...
describe("background lock lifecycle conformance", () => {
  it("explicit lock and auto-lock both clear dApp-visible profiles", async () => {
    ...
    expect(source).toMatch(
      /message\?\.type === "DUSK_UI_LOCK"[\s\S]*?lockWallet\(\)[\s\S]*?broadcastProfilesChangedAll/
    );
    expect(source).toMatch(
      /if \(elapsed >= timeoutMs\) \{[\s\S]*?lockWallet\(\)[\s\S]*?broadcastProfilesChangedAll/
    );
    expect(source).toMatch(
      /async function lockWallet\(\)[\s\S]*?rejectAllPendingApprovals\(\)[\s\S]*?engineCall\("engine_lock"\)/
    );
  });
