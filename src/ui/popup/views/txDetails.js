...
export function txDetailsView(ov, { state, actions } = {}) {
  ...
  const detailsRows = [
    ...
    tx?.gasSpent ? kvRow("Gas spent", tx.gasSpent, { mono: true }) : null,
    tx?.blockHeight ? kvRow("Block height", tx.blockHeight, { mono: true }) : null,
    tx?.blockHash ? kvRow("Block hash", tx.blockHash, { mono: true }) : null,
    tx?.finalizedAt ? kvRow("Finalized", fmtDate(tx.finalizedAt)) : null,
